...
function DoBuy()
    ...
    if last_entry ~= main_source:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
...
function DoClose()
    if last_exit ~= main_source:date(NOW) then
        if AllowTrade then
            CloseTrades("B");
            CloseTrades("S");
        end
        Signal("Exit", main_source);
